...
\begin{proof}
    Ver \cite{johnpfaffen}, teorema 4.5, página 14. \blackproof
\end{proof}
...
\begin{proof} 
    Como $1 \in A$ para todo $A \in \mathcal{A}$, temos $1 \in \bigcap \mathcal{A}$. Agora, se $n \in \bigcap \mathcal{A}$, então $n \in A$ para todo $A \in \mathcal{A}$; como cada $A \in \mathcal{A}$ é indutivo, temos $n + 1 \in A$, donde $n+1 \in \bigcap \mathcal{A}$. \blackproof
\end{proof}
...
\begin{proof}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Se $A$ é um conjunto indutivo, então, pela definição de $\N$, temos $\N \subseteq A$. Daí, se $A \subseteq \N$, então $A = \N$. \blackproof
            \item Definindo $A := \{ n \in \N : s(n)\}$, temos $A \subseteq \N$. Além disso, $1 \in A$ e $n+1 \in A$ sempre que $n \in A$, de modo que $A$ é indutivo. Com isso, $\N \subseteq A$, de modo que $A = \N$, isto é, vale $s(n)$ para todo $n \in \N$. \blackproof
        \end{enumerate}
\end{proof}
...
\begin{proof}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Fixe $m \in \N$ e defina $A := \{n \in \N : m + n \in \N \}$. Temos $1 \in A$ pois se $m \in \N$ então $m+1 \in \N$ já que $\N$ é indutivo. Agora, se $n \in A$, então $m + n \in \N$, e como $\N$ é indutivo vem $(m+n)+1 \in \N$, isto é, $m+(n+1) \in \N$, de modo que $n+1 \in A$. Com isso, $A$ é indutivo, isto é, $\N \subseteq A$. Como $A \subseteq \N$ pela definição de $A$, segue que $A = \N$. Como $m$ foi fixado arbitrariamente, segue que $m + n \in \N$ para quaisquer $m,n \in \N$. \blackproof
            \item Fixe $m \in \N$ e defina $A := \{n \in \N : m \cdot n \in \N \}$. Temos $1 \in A$ pois $m \cdot 1 = m \in \N$. Agora, se $n \in A$, então $m \cdot n \in \N$, e pelo item anterior $m \cdot n + m \in \N$, isto é, $m \cdot (n+1) \in \N$, de modo que $n+1 \in A$. Com isso, $A$ é indutivo, isto é, $\N \subseteq A$. Como $A \subseteq \N$ pela definição de $A$, segue que $A = \N$. Como $m$ foi fixado arbitrariamente, segue que $m \cdot n \in \N$ para quaisquer $m,n \in \N$. \blackproof
            \item Definindo $A := \{ n \in \N : n \geq 1\}$, temos $A \subseteq \N$. Claramente $1 \in A$ pois $1 \geq 1$. Agora, se $n \in A$, então $1>0 \Rightarrow n+1 > n \geq 1 \Rightarrow n+1 \geq 1$, de modo que $n+1 \in A$. Com isso $\N \subseteq A$, donde $A = \N$. \blackproof
        \end{enumerate}
\end{proof}
...
\begin{proof}
    Provemos que $X := \N - A$ é vazio. De fato, se $X \neq \emptyset$, então pela Boa Ordenação existiria $p \in X$ mínimo; daí, todo $m < p$ seria $m \in A$, de modo que, pela definição de $A$, $p \in A$, absurdo! Logo, $X = \emptyset$. \blackproof
\end{proof}
...
\begin{proof}
    Ver \cite{cursodeanalise1}, teorema 3 do capítulo 3. \blackproof    
\end{proof}

\subsection{A Unicidade dos Números Naturais}

\begin{teo}[da Recursão] \label{teo.reais:recursão}
    Sejam $(\F, +_{\F}, \cdot_{\F}, \leq_{\F})$ um corpo ordenado, $X$ um conjunto, $a \in X$ e $f:X \to X$ uma função. Existe e é única a função $\varphi : \N_{\F} \to X$ tal que
        \begin{itemize}
            \item $\varphi(1_{\F}) = a$;
...
\begin{proof}
    Para simplificar a notação vamos omitir o índice $_{\F}$. Sendo $\mathcal{C}$ o conjunto de todos as relações de $\N$ em $X$ que tem as propriedades desejadas, deve existir um único elemento de $\mathcal{C}$ que seja uma função de domínio $\N$. Formalmente,
        \[
            \mathcal{C} := \{R \in \mathcal{P}{(\N \times X)} : (1,a) \in R \land \forall n \forall x ((n,x) \in R \Rightarrow (n+1, f(x)) \in R)\}.
        \]
    Como $\N \times X \in \mathcal{C}$, temos $\mathcal{C} \neq \emptyset$, logo existe $\varphi := \bigcap \mathcal{C}\in \mathcal{C}$. Afirmamos que $\varphi$ é esse único elemento procurado. Provemos então que $\varphi$ é uma função de domínio $\N$. Para tanto, $\varphi$ deve ter as seguintes propriedades:
        \begin{itemize}
            \item A relação $\varphi$ tem domínio $\N$: $\forall n (n \in \N \Rightarrow \exists x (x \in X \land  (n,x) \in \varphi ))$.
            \item A relação $\varphi$ é funcional: $\forall n \forall x \forall y ((n,x) \in \varphi \land (n,y) \in \varphi \Rightarrow x=y )$.
        \end{itemize}
    Sendo $S := \{n \in \N : \exists x ((n,x) \in \varphi \land \forall y ((n,y) \in \varphi \Rightarrow y=x ) ) \}$, se $S = \N$, então $\varphi$ terá essas propriedades. Como $S \subseteq \N$, basta provar que $\N \subseteq S$, e para isso basta provar que $S$ é um conjunto indutivo.
        \begin{enumerate}[label=\roman*.]
            \item Como $(1,a) \in \varphi$, provemos a unicidade de $a$ como imagem de $1$ por $\varphi$. Se existisse $b \in X$ tal que $(1,b) \in \varphi$ e $b \neq a$, então $(1,a) \in \varphi \setminus \{(1,b)\}$, e se $(n,x) \in \varphi \setminus \{(1,b)\}$, então $(n,x) \in \varphi$, de modo que $(n+1,f(x)) \in \varphi$; como $n+1 \neq 1$, teríamos $(n+1, f(x)) \in \varphi \setminus \{(1,b)\}$, de modo que $\varphi \setminus \{(1,b)\} \in \mathcal{C}$, uma contradição. Com isso, $1 \in S$.
            \item Se $n \in S$, então existe um único $x \in X$ tal que $(n,x) \in \varphi$. Com isso, vem $(n+1,f(x)) \in \varphi$; provemos então a unicidade de $f(x)$ como imagem de $n+1$ por $\varphi$. Suponha que exista $b \in X$ tal que $(n+1,b) \in \varphi$ e $b \neq f(x)$. Temos $(1,a) \in \varphi \setminus \{(n+1,b)\}$ pois $n+1 \neq 1$. Se $(m,y) \in \varphi \setminus \{(n+1,b)\}$, então $(m,y) \in \varphi$ e $(m+1,f(y)) \in \varphi$. Se $m=n$, então $y=x$, $m+1 = n+1$ e $f(y) = f(x) \neq b$, de modo que $(m+1, f(x)) \in \varphi \setminus \{(n+1, b)\}$. Se $m \neq n$, então $m+1 \neq n+1$, de modo que $(m+1, f(x)) \in \varphi \setminus \{(n+1, b)\}$. Com isso, vem $\varphi \setminus \{(n+1, b)\} \in \mathcal{C}$, uma contradição. Logo, se $n \in S$, então $n+1 \in S$, de modo que $S = \N$.
        \end{enumerate}
    Com isso, fica provada a existência de uma função $\varphi : \N \to X$ com as propriedades desejadas. Provemos, por fim, sua unicidade. Sendo $\psi : \N \to X$ uma função com essas mesmas propriedades e definindo $T := \{ n \in \N : \psi(n) = \varphi(n)\}$, claramente $1 \in T$, e se $n \in T$, então $\psi(n) = \varphi(n)$, de modo que $\psi(n+1) = f(\psi(n)) = f(\varphi(n)) = \varphi(n+1)$, isto é, $n+1 \in T$. Com isso, vem $T = \N$, o que prova a unicidade de $\varphi$. \blackproof
\end{proof}

\begin{teo}
    Sejam $(\F_1, +_1, \cdot_1, \leq_1)$ e $(\F_2, +_2, \cdot_2, \leq_2)$ corpos ordenados. Existe uma única bijeção $\varphi : \N_{\F_1} \to \N_{\F_2}$ tal que, para quaisquer $m,n \in \N_{\F_1}$, valem as seguintes propriedades.
        \begin{enumerate}[label=\roman*.]
            \item $\varphi (m +_1 n) = \varphi(m) +_2 \varphi(n)$.
            \item $\varphi (m \cdot_1 n) = \varphi(m) \cdot_2 \varphi(n)$.
...
\begin{proof}
    Tomando $X = \N_{\F_2}$, $a = 1_{\F_2}$ e $f: \N_{\F_2} \to \N_{\F_2}$ dada por $f(n) = n +_2 1_{\F_2}$ no teorema da recursão \eqref{teo.reais:recursão}, existe uma única função $\varphi : \N_{\F_1} \to \N_{\F_2}$ tal que $\varphi(1_{\F_1}) = 1_{\F_2}$ e $\varphi(n +_1 1_{\F_1}) = f[\varphi(n)] = \varphi(n) +_2 1_{\F_2}$. Provemos a bijetividade de $\varphi$ construindo explicitamente sua função inversa. Tomando $X = \N_{\F_1}$, $a = 1_{\F_1}$ e $g: \N_{\F_1} \to \N_{\F_1}$ dada por $g(n) = n +_1 1_{\F_1}$ no teorema da recursão \eqref{teo.reais:recursão}, existe uma única função $\psi : \N_{\F_2} \to \N_{\F_1}$ tal que $\psi(1_{\F_2}) = 1_{\F_1}$ e $\psi(n +_2 1_{\F_2}) = g[\psi(n)] = \psi(n) +_1 1_{\F_1}$. Vamos provar que $\psi \circ \varphi = \id_{\F_1}$ e $\varphi \circ \psi = \id_{\F_2}$. Sendo $F := \psi \circ \varphi : \N_{\F_1} \to \N_{\F_1}$, temos
        \[
            F(1_{\F_1}) = \psi[\varphi(1_{\F})] = \psi(1_{\F_2}) = 1_{\F_1},
        \]
    e ainda,
        \[
            F(n +_1 1_{\F_1}) = \psi[\varphi(n +_1 1_{\F})] = \psi[\varphi(n) +_2 1_{\F_2}] = F(n) +_1 1_{\F_1}.
        \]
    Agora, como $\id_{\N_{\F_1}}(1_{\F_1}) = 1_{\F_1}$ e $\id_{\N_{\F_1}}(n +_1 1_{\F_1}) = \id_{\N_{\F_1}}(n) +_1 1_{\F_1}$, e como só existe uma função com essas propriedades, vem $F = \id_{\N_{\F_1}}$. A prova de que $\varphi \circ \psi = \id_{\N_{\F_2}}$ é completamente análoga, de modo que $\psi$ é a função inversa de $\varphi$ e, portanto, $\varphi$ é bijetiva.
        \begin{enumerate}[label=\roman*.]
            \item Fixe $m \in \N_{\F_1}$ e defina $A := \{n \in \N_{\F_1} : \varphi(m +_1 n) = \varphi(m) +_2 \varphi(n) \}$. Como $\varphi(m +_1 1_{\F_1}) = \varphi(m) +_2 1_{\F_2}$, temos $1_{\F_1} \in A$ pois $\varphi$ foi obtida pelo teorema da recursão. Agora, se $n \in A$, então $\varphi(m +_1 n) = \varphi(m) +_2 \varphi(n)$ e
                \begin{align*}
                    \varphi[m +_1 (n +_1 1_{\F_1})] &= \varphi[(m +_1 n) +_1 1_{\F_1}] \\
                    &= \varphi[(m +_1 n)] +_2 1_{\F_2} \\
                    &= [\varphi(m) +_2 \varphi(n)] +_2 1_{\F_2} \\
                    &= \varphi(m) +_2 [\varphi(n) +_2 1_{\F_2}] \\
                    &= \varphi(m) +_2 \varphi(n +_1 1_{\F_1}),
                \end{align*}
            de modo que $n +_1 1_{\F_1} \in A$, isto é, $A$ é indutivo. Logo $A = \N$.
            \item Fixe $m \in \N_{\F_1}$ e defina $A := \{n \in \N_{\F_1} : \varphi(m \cdot_1 n) = \varphi(m) \cdot_2 \varphi(n) \}$. Como
                \[
                    \varphi(m \cdot_1 1_{\F_1}) = \varphi(m) = \varphi(m) \cdot_2 1_{\F_2} = \varphi(m) \cdot_2 \varphi(1_{\F_1}),
                \]
            temos $1_{\F_1} \in A$. Agora, se $n \in A$, então $\varphi(m \cdot_1 n) = \varphi(m) \cdot_2 \varphi(n)$ e
                \begin{align*}
                    \varphi[m \cdot_1 (n +_1 1_{\F_1})] &= \varphi(m \cdot_1 n +_1 m \cdot_1 1_{\F_1}) \\
                    &= \varphi(m \cdot_1 n) +_2 \varphi(m) \\
                    &= \varphi(m) \cdot_2 \varphi(n) +_2 \varphi(m) \\
                    &= \varphi(m) \cdot_2 (\varphi(n) +_2 1_{\F_2}) \\
                    &= \varphi(m) \cdot_2 [\varphi(n +_1 1_{\F_1})],
                \end{align*}
            de modo que $n +_1 1_{\F_1} \in A$, isto é, $A$ é indutivo. Logo $A = \N$.
            \item Se $m = n$, então $\varphi(m) = \varphi(n)$, e se $\varphi(m) = \varphi(n)$, então $m=n$ pela injetividade de $\varphi$. Com isso, basta provarmos $m <_1 n \Leftrightarrow \varphi(m) <_2 \varphi(n)$. Se $m <_1 n$, então pelo lema \eqref{lema.reais:discretude} existe $k \in \N_{\F_1}$ tal que $n = m +_1 k$. Com isso, $\varphi(n) = \varphi(m) +_2 \varphi(k)$, e se fosse $\varphi(m) \geq_2 \varphi(n)$, teríamos $\varphi(k) \leq_2 0_{\F_2}$, um absurdo. Agora, se $\varphi(m) <_2 \varphi(n)$, então $m \neq n$ pela injetividade de $\varphi$, e se fosse $m >_1 n$ existiria $k \in \N_{\F_1}$ tal que $m = n +_1 k$, de modo que $\varphi(n +_1 k) <_2 \varphi(n)$, uma contradição.
        \end{enumerate}
    Com isso, vemos que a função $\varphi : \N_{\F_1} \to \N_{\F_2}$, obtida pelo teorema da recursão, é bijetiva, preserva a adição, a multiplicação e a ordem, como queríamos. \blackproof
\end{proof}

\subsection{Definições recursivas}

O objetivo desta subseção é usar o teorema da recursão \eqref{teo.reais:recursão} para fazermos definições recursivas.
    ...
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            ...
            \item Basta ver que esse enunciado é a contrapositiva do item anterior. No entanto, ainda assim, daremos uma outra prova, que se dará por indução em $n$. Evidentemente, o resultado vale para $n=1$. Agora, supondo que o resultado vale para $n \in \N$, tomando uma bijeção $f : [n+1] \to A \subseteq [n+1]$ provaremos que $A = [n+1]$. Sendo $a := f(n+1)$, a restrição $\tilde{f} : [n] \to A \setminus \{a \}$ é uma bijeção.
                ...
                \end{itemize}
            Com isso, temos $A = [n+1]$ em ambos os casos, como queríamos provar. \blackproof
        \end{enumerate}
\end{proof}
...
\begin{proof}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            ...
            Uma outra prova é o que segue. Se $h = g^{-1} \circ f :[m] \to [n]$ é uma bijeção, então $m=n$. De fato, se $m \leq n$, então $[m] \subseteq [n]$, e como $h^{-1} : [n] \to [m] \subseteq [n]$, pelo teorema \eqref{teo:1} só pode ser $[m]= [n]$, donde $m=n$. \blackproof
            ...
            ($\Leftarrow$) Existindo uma bijeção $f : A \to B$, temos que $g^{-1} \circ f^{-1} \circ h : [m] \to [n]$ é também uma bijeção, donde $m=n$, isto é, $|A|=|B|$. \blackproof
        \end{enumerate}
\end{proof}
...
\begin{proof}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]         
            \item Suponha que existe uma tal bijeção. Se $X$ é finito, então existe uma bijeção $h : [n] \to X$ para algum $n \in \N$. Definindo $A := h^{-1}(Y)$, temos $A \subsetneq [n]$ e, além disso, a restrição de $h$ a $A$ é uma bijeção $h_A : A \to Y$. Com isso, a composta $h^{-1} \circ f^{-1} \circ h_A : A \to [n]$ é uma bijeção de $A \subsetneq [n]$ em $[n]$, o que contraria o teorema \eqref{teo:1}! Logo, não pode existir uma bijeção $f:X \to Y$ onde $X$ é finito e $Y \subsetneq X$. \blackproof
...
\begin{defi}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Um conjunto $X \subseteq \N$ é \textit{limitado} se existe $n \in \N$ tal que $x \leq n$ para todo $x \in X$.
...
\begin{defi}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            ...
            \item Um subconjunto $X \subseteq \N$ é \textit{ilimitado} se ele não é limitado, isto é, se para todo $n \in \N$ existe $p \in X$ tal que $p > n$.
        \end{enumerate}
\end{defi}
...
\begin{defi}
    Um conjunto $X$ é \textit{enumerável} se é finito ou se existe uma bijeção $f : \N \to X$. A função $f$ é uma \textit{enumeração} de $X$.
\end{defi}
...
\begin{proof}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Sejam $\alpha$ e $\beta$ supremos de $S$. Como $\alpha$ é a menor das cotas superiores de $S$ e $\beta$ é uma cota superior de $S$, temos $\alpha \leq \beta$. Como $\beta$ é a menor das cotas superiores de $S$ e $\alpha$ é uma cota superior de $S$, temos $\beta \leq \alpha$. Logo $\alpha = \beta$. \blackproof
    ...
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Segue facilmente por contradição.
                \begin{enumerate}[label=\roman*.]
                    \item Do contrário, seria $x \leq \beta < \sup{S}$ para todo $x \in S$, de modo que $\beta$ seria uma cota superior de $S$ menor que $\sup{S}$, um absurdo. \blackproof
                    \item Do contrário, existiria $\epsilon \in \F_{>0}$ tal que $x \leq \sup{S} - \epsilon < \sup{S}$ para todo $x \in S$, de modo que $\sup{S} - \epsilon$ seria uma cota superior de $S$ menor que $\sup{S}$, um absurdo. \blackproof
                \end{enumerate}
            \item Segue analogamente. \blackproof
        \end{enumerate}
\end{proof}
...
\begin{proof}
    Ver \cite{ajwhite}, teorema 1-10, página 32. Ver \cite{stromberg}, corolário 1.12, página 13. \blackproof
\end{proof}
...
\begin{proof}
    ...
    Suponha, por absurdo, que $\N_{\F}$ seja limitado superiormente. Como $\N_{\F} \in \mathcal{P}(\F)_{\neq \emptyset}$, existe $a := \sup{\N_{\F}}$. Por \eqref{teo.reais:supinf}, para $\epsilon = 1$ existe $n \in \N_{\F}$ tal que $a-1 < n$, isto é, $a < n+1$, e como $n+1 \in \N_{\F}$, temos uma contradição. \blackproof
\end{proof}
        ...
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Se $e' \in G$ é um elemento neutro de $*$, então
                \[
                    e = e * e' = e' * e = e',
                \]
            como havíamos afirmado. \blackproof
            \item Segue analogamente. \blackproof
        \end{enumerate}
\end{proof}
        ...
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Se $0' \in A$ é um elemento neutro de $+$, então
                \[
                    0' = 0' + 0 = 0 + 0' = 0,
                \]
            conforme afirmado. \blackproof
            \item
            \item Observando que $a \cdot 0 = a \cdot (0+0) = a \cdot 0 + a \cdot 0$, somando $-(a \cdot 0)$ aos dois lados de $ a \cdot 0 = a \cdot 0 + a \cdot 0$, segue que $0 \cdot a = 0$. \blackproof
        \end{enumerate}
\end{proof}
