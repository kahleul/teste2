...
\section{Corpos Ordenados Completos}
...
\begin{prop}
    Seja $\F$ um corpo. Valem as seguintes afirmações.
        ...
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            ...
            \item Existe um subconjunto $\F_{>0} \subseteq \F$ tal que
                \begin{enumerate}[label=\roman*.]
                    ...
                    \item se $x \in \F$, então ou $x=0$, ou $x \in \F_{>0}$, ou $-x \in \F_{>0}$.
                \end{enumerate}
        \end{enumerate}
\end{prop}
...
\begin{prop}
    Sejam $\F$ um corpo ordenado e $S \in \mathcal{P}(\F)_{\neq \emptyset}$.
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
...
\begin{defi}
    Um corpo ordenado $(\F,+,\cdot, \leq)$ é \textit{completo} se todo subconjunto não vazio de $\F$ limitado superiormente possui supremo em $\F$.
\end{defi}
...
\begin{cor}
    Todo subconjunto não vazio de $\R$ limitado inferiormente possui ínfimo em $\R$.
\end{cor}
...
\begin{teo}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Seja $S \in \mathcal{P}(\R)_{\neq \emptyset}$ limitado superiormente.
                \begin{enumerate}[label=\roman*.]
                    \item Se $\beta \in \R$ e  $\beta < \sup{S}$, então existe $x \in S$ tal que $x > \beta$.
                    \item Para todo $\epsilon \in \R_{>0}$ existe $x \in S$ tal que $\sup{S} - \epsilon < x$.
                \end{enumerate}
            \item Seja $S \in \mathcal{P}(\R)_{\neq \emptyset}$ limitado inferiormente.
                \begin{enumerate}[label=\roman*.]
                    \item Se $\beta \in \R$ e  $\inf{S} < \beta$, então existe $x \in S$ tal que $x < \beta$.
                    \item Para todo $\epsilon \in \R_{>0}$ existe $x \in S$ tal que $x < \inf{S} + \epsilon$.
                \end{enumerate}
        \end{enumerate}
\end{teo}
...
\begin{defi}
    Um subconjunto $I \subseteq \R$ é \textit{indutivo} se $1 \in I$ e $n \in I \Rightarrow n+1 \in I$. Isso é denotado por $\ind{I}$.
\end{defi}

\begin{ex}
    $\R$ é um conjunto indutivo. Com isso, o conjunto de todos os subconjuntos indutivos de $\R$ é não vazio, isto é, $\{ I \in \mathcal{P}{(\R)} : \ind{(I)} \} \neq \emptyset$. Em particular, isso nos permite considerar $\bigcap \{ I \in \mathcal{P}{(\R)} : \ind{(I)} \}$.
\end{ex}

\begin{prop}
    Se $\mathcal{A}$ é uma coleção não vazia de subconjuntos indutivos de $\R$, isto é, se 
        $ \ds
            \mathcal{A} \subseteq \{ I \in \mathcal{P}{(\R)} : \ind{(I)} \}
        $
    e $\mathcal{A} \neq \emptyset$, então $\bigcap \mathcal{A}$ é um conjunto indutivo.
\end{prop}
...
\begin{defi}
    O conjunto dos números naturais, denotado por $\N$, é definido como a interseção de todos os subconjuntos indutivos de $\R$:
        \[
            \N := \bigcap \{ I \in \mathcal{P}{(\R)} : \ind{(I)} \}.
        \]
    Em particular, $\N_0 := \N \cup \{0\}$.
\end{defi}

\begin{teo}[Indução fraca]
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Se um subconjunto $A \subseteq \N$ é indutivo, então $A = \N$.
            \item Seja $s(n)$ uma proposição bem definida para cada $n \in \N$. Se $s(1)$ é verdadeira e se $s(n) \Rightarrow s(n+1)$ é verdadeira, então $s(n)$ é verdadeira para todo $n \in \N$.
        \end{enumerate}
\end{teo}
...
\begin{prop}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Para quaisquer $m,n \in \N$, tem-se $m+n \in \N$ e $m \cdot n \in \N$.
...
\begin{proof}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Definindo $G := \{ m \in \N : \forall n (n \in \N \Rightarrow n+m \in \N )\}$, temos $G \subseteq \N$. Temos que $1 \in G$ pois, para todo $n \in \N$, vale $n \in \N \Rightarrow n+1 \in \N$, já que $\N$ é indutivo. Agora, se $n+m \in \N$, então, como $\N$ é indutivo, vale $(n+m)+1 \in \N$, isto é, $n+(m+1) \in \N$. Assim, $m \in G \Rightarrow m+1 \in G$, isto é, $G$ é indutivo e, portanto, cumpre $\N \subseteq G$, donde $G = \N$. Agora, definindo $H := \{ m \in \N : \forall n (n \in \N \Rightarrow n \cdot m \in \N )\}$, temos $H \subseteq \N$. Temos que $1 \in H$ pois, para todo $n \in \N$, vale $n \cdot 1 = n \in \N$. Agora, se $n \cdot m \in H$, então $n \cdot (m+1) \in H$ pois $n \cdot (m+1) = n \cdot m + n \cdot 1$ e, como vimos, a soma de dois naturais é um natural. Com isso, $H$ é indutivo e, portanto, cumpre $\N \subseteq H$, donde $H = \N$. \itemproof
...
\begin{lem}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Para qualquer $n \in \N$, ou $n-1=0$ ou $n-1 \in \N$.
            \item Para quaisquer $m,n \in \N$, com $m<n$, temos $n-m \in \N$.
            \item Para todo $n \in \N$ não existe $m \in \N$ para o qual $n < m < n+1$.
        \end{enumerate}
\end{lem}
...
\begin{proof}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Definindo $A := \{ n \in \N : n-1=0 \lor n-1 \in \N \}$, temos $A \subseteq \N$. Temos $1 \in A$ pois vale $1 - 1 = 0 \lor 1 - 1 \in \N$. Agora, se $n \in A$, então $(n+1)-1 = n \in A$, de modo que $n+1 \in A$. Com isso, $A$ é indutivo, de modo que $\N \subseteq A$, donde $A = \N$. \itemproof
            \item Definindo $A := \{ m \in \N : \forall n (n \in \N \land m < n \Rightarrow n - m \in \N) \}$, temos $A \subseteq \N$. Sendo $n \in \N$ com $n>1$, temos $n-1 > 0$, isto é, $n-1 \neq 0$; com isso, pelo item anterior, $n-1 \in \N$. Agora, supondo que $n,m \in \N$ e $m<n$ implicam $n-m \in \N$, provemos que $n,m \in \N$ e $m+1<n$ implicam $n-(m+1) \in \N$. De fato, $m+1 < n \Rightarrow m < n-1$; pelo item anterior, $n-1 \in \N$, donde, pela hipótese de indução, $(n-1) - m \in \N$, isto é, $n - (m+1) \in \N$. Isso nos mostra que $m \in A \Rightarrow m+1 \in A$, isto é, que $A$ é indutivo; daí, $\N \subseteq A$, donde $A = \N$. \itemproof
...
\begin{teo}[Princípio da Boa Ordenação] Todo subconjunto não vazio de números naturais possui um menor elemento. Isto é, se $\emptyset \neq A \subseteq \N$, então existe $a \in A$ tal que $a \leq x$ para todo $x \in A$.
\end{teo}

\begin{proof}
    Defina $[n] := \{ x \in \N \mid x \leq n \}$ e $X := \{ n \in \N \mid [n] \subseteq \N - A\}$. Se $1 \in A$, então $1 = \min A$. Se $1 \notin A$, então $[1] = \{ 1 \} \subset \N - A$, de modo que $1 \in X$. Agora, como $A \neq \emptyset$, temos que $X \neq \N$. Se $1 \in X$ e $X \neq \N$, então existe $n_0 \in X$ tal que $n_0 + 1 \notin X$, isto é, $[n_0] \cap A = \emptyset$ e $[n_0+1] \cap A \neq \emptyset$. Com isso, $n_0 + 1 \in A$, sendo este o menor elemento de $A$ (pois não existe um natural entre $n_0$ e $n_0 + 1$). \itemproof
\end{proof}

\begin{proof}
    Suponha por contradição que existe um conjunto $\emptyset \neq A \subseteq \N$ que não tem um elemento mínimo. Definindo $X := \{ n \in \N : [n] \cap A = \emptyset \}$, veja que $1 \in X$ (de fato, se $1 \notin X$, então  $[1] \cap A \neq \emptyset$, de modo que $1 \in A$ seria o menor elemento de $A$, absurdo!). Agora, se $n \in X$, então $[n] \cap A = \emptyset$; daí, se fosse $n+1 \in A$, este seria o menor elemento de $A$, absurdo! Logo, só pode ser $n+1 \in X$, de modo que $X$ é indutivo e $\N \subset X$. Como $X \subset \N$ por definição, temos que $X = \N$, donde $A = \emptyset$, uma contradição. \itemproof
\end{proof}
...
\begin{prop}
    Seja $X$ um conjunto finito.
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Todo subconjunto próprio $Y \subsetneq X$ não admite uma bijeção $f : X \to Y$.
...
\begin{defi}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Um conjunto $X$ é dito \textit{infinito} quando ele não é finito, isto é, quando $X \neq \emptyset$ e quando, para cada $n \in \N$, não existe uma bijeção $f : [n] \to X$.
...
\section{Inteiros e Racionais}

\begin{defi}
    O conjunto dos números inteiros, denotado por $\Z$, é definido como $\Z := \N_0 \cup -\N$, em que $-\N := \{-n : n \in \N\}$.
\end{defi}

\begin{prop}
    Para quaisquer $m,n \in \Z$, temos $m+n \in \Z$ e $m \cdot n \in \Z$.
\end{prop}
...
\begin{defi}
    O conjunto dos números racionais, denotado por $\Q$, é definido como $\Q := \{ p \cdot q^{-1} : p \in \Z, q \in \Z_{\neq 0} \}$. Denotaremos $p \cdot q^{-1}$ por $\frac{p}{q}$.
\end{defi}
