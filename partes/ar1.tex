...
\begin{proof}
    Absolutamente trivial. \blackproof
\end{proof}
...
\begin{defi}
    Uma \textit{vizinhança} de um ponto $a \in \R$ com raio $r \in \R_{>0}$ é definida como
        \[
            V_r(a) := \{x \in \R : |x-a|<r \}.
        \]
\end{defi}
...
\begin{defi}
    Um \textit{ponto de acumulação} de um subconjunto $A \subseteq \R$ é um ponto $a \in \R$ tal que 
        \[
            V_{\delta}(a) \cap A_{\neq a} \neq \emptyset
        \]
...
\begin{proof}
    \textbf{(a)} Basta tomar $\epsilon = L$. \blackproof
...
\begin{proof}
    ...
    Como, por hipótese, $x \in \left] a,b \right[ \Rightarrow | f(x) - f(p) | < \epsilon$, temos então que
        ...
        \]
    Como $ x \in \left] p - \delta, p + \delta \right[ \Leftrightarrow |x-p| < \delta $, vemos que para todo $\epsilon>0$ existe um $\delta > 0$ tal que $|x-p| < \delta \Rightarrow |f(x) - f(p)| < \epsilon$, isto é, $f$ é contínua em $p$. \blackproof
    ...
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            ...
            Observando que
                ...
                \]
            a conclusão segue. \blackproof
            \item Tomando $\epsilon = -f(a)$, segue analogamente. \blackproof
        \end{enumerate}
\end{proof}
...
\begin{proof}
    ...
    Temos $S \neq \emptyset$ pois $a \in S$ já que $f(a) < 0$. $S$ é limitado pois $S \subsetneq [a,b]$. Assim, existe $p := \sup{S}$. Provemos que $f(p) = 0$. De fato, pela tricotomia de $<$ em $\R$, ou $f(p) > 0$, ou $f(p) < 0$, ou $f(p) = 0$. 
        ...
        \end{itemize}
    Logo, só pode ser $f(p) = 0$. Além disso, como $p \in [a,b]$ e $f(a) < 0$ e $f(b) > 0$, temos que $p \in (a,b)$. \blackproof
\end{proof}
...
\begin{proof}
    \textbf{(a)} Segue como corolário da conservação do sinal \eqref{consin2}. \blackproof

    \textbf{(b)} Basta aplicar o resultado do item (a) na função $h(x) := f(x) - g(x)$. \blackproof
...
\begin{proof} 
    \textbf{(a)} Sabemos que $(1+\epsilon)^n \geq 1 + n\epsilon$ para todo natural $n \geq 1$. Pois tome $n$ tal que $1 + n\epsilon > a$ (basta que $ n > \frac{a-1}{\epsilon}$); daí, $(1+\epsilon)^n>a$, donde $a^{\frac{1}{n}} - 1 < \epsilon$. \blackproof

    \textbf{(b)} Para racionais $t>x$ temos $a^r<a^t$ para todo racional $r<x$. Pelo item anterior, existe um natural $n$ para o qual $a^{\frac{1}{n}} - 1 < \epsilon \cdot a^{-t}$, donde $a^t (a^{\frac{1}{n}}-1) < \epsilon$. Tomando racionais $r$ e $s$, com $r<x<s$, para os quais $s-r < 1/n$, temos $a^s - a^r = a^r (a^{s-r} -1) < a^t (a^{\frac{1}{n}} - 1) < \epsilon$. \blackproof
...
\begin{proof}
    \textbf{(a)} Provemos que para quaisquer $a,b \in I$ temos $a<b \Rightarrow f(a) < f(b)$. Sejam, então, $a,b \in I$ com $a<b$. Evidentemente, temos que $f$ e contínua em $[a,b]$ e derivável em $]a,b[$; daí, pelo Teorema do Valor Médio \eqref{teo:rolletvm}, existe $c \in \left] a,b \right[$ tal que 
        $
            f(b) - f(a) = f'(c) (b-a).
        $
    Como $f'(c) > 0$ e $b>a$, temos que $f(b) - f(a) > 0$, donde $f(a) < f(b)$. \blackproof
    ...
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Para todo $i \in [n]$ temos $m_i \leq M_i$ e $\Delta x_i > 0$. Logo $m_i \Delta x_i \leq M_i \Delta x_i$ para todo $i \in [n]$. Tomando a soma, temos que
                ...
                \]
            de modo que $L(f,P) \leq U(f,P)$, como havíamos afirmado. \blackproof
...
\begin{proof}
    ...
    Como $|b_i - a_i| \leq \Delta x_i < \delta$ para todo $i \in [n]$, pela continuidade uniforme de $f$ temos $\ds M_i - m_i = |f(b_i) - f(a_i)| < \dfrac{\epsilon}{b-a}$ para todo $i \in [n]$, de modo que
        ...
        \]
    Assim, pelo critério de integrabilidade, $f$ é integrável. \blackproof
...
\begin{proof}
    ...
    Agora, a partição $P : a = x_0 < \cdots < x_n = b$ definida por $x_i = a +  i \cdot \frac{b-a}{n}$ para todo $i \in [n] \cup \{ 0\}$ é tal que $\Delta x_i = \frac{b-a}{n}$,  $M_i = f(x_i)$ e $m_i = f(x_{i-1})$ (pois $f$ é crescente), donde
        \begin{align*}
            ...
            &<\epsilon.
        \end{align*}
    Assim, pelo critério de integrabilidade, $f$ é integrável. No caso em que $f$ é decrescente, a demonstração é análoga. \blackproof
...
\begin{proof}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Precisamos provar que para todo $\epsilon \in \R_{>0}$ existe $\delta \in \R_{>0}$ tal que
                \[
                    |x-y| < \delta \Rightarrow |F(x) - F(y)| < \epsilon
                \]
            ...
            \item Para provar que $F$ é derivável em $x_0$ e $F'(x_0) = f(x_0)$, basta provar que
                \[
