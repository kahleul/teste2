...
\item Suponha que $\N$ seja limitado superiormente. Como $\N \subsetneq \R$, existe $a := \sup{\N}$. Como $a-1$ não é cota superior de $\N$, existe $n \in \N$ tal que $a-1 < n$, donde $a < n+1 \in \N$, absurdo! Em particular, concluímos que para todo $x \in \R$ existe $n \in \N$ tal que $n > x$. \blackproof
\item Pelo item anterior, para o número $\frac{b}{a}$ existe $n \in \N$ tal que $n > \frac{b}{a}$, e sendo $a>0$, temos $na>b$. \blackproof
\item Basta colocar $b=1$ e $a= \epsilon$ no item anterior: existe $n \in \N$ tal que $n \cdot \epsilon > 1$, donde $\frac{1}{n} < \epsilon$. \blackproof
\item Se fosse $x > a$, então a propriedade arquimediana nos diz que existe $n \in \N$ tal que $n \cdot (x-a) > y$, donde $x > a + \frac{y}{n}$, o que contraria a hipótese. Logo, só pode ser $x=a$. \blackproof
...
Agora, supondo que $n,m \in \N$ e $m<n$ implicam $n-m \in \N$, provemos que $n,m \in \N$ e $m+1<n$ implicam $n-(m+1) \in \N$. De fato, $m+1 < n \Rightarrow m < n-1$; pelo item anterior, $n-1 \in \N$, donde, pela hipótese de indução, $(n-1) - m \in \N$, isto é, $n - (m+1) \in \N$. Isso nos mostra que $m \in A \Rightarrow m+1 \in A$, isto é, que $A$ é indutivo; daí, $\N \subseteq A$, donde $A = \N$. \blackproof

\begin{proof}
    \leavevmode
        \begin{enumerate}[leftmargin=*, align=left]
            \item[\textbf{(a)} $\Rightarrow$ \textbf{(b)}:] \blackproof
            \item[\textbf{(b)} $\Rightarrow$ \textbf{(c)}:] \blackproof
            \item[\textbf{(c)} $\Rightarrow$ \textbf{(d)}:] \blackproof
            \item[\textbf{(d)} $\Rightarrow$ \textbf{(a)}:] \blackproof  
        \end{enumerate}
\end{proof}
...
\begin{enumerate}[label=\roman*.]
        ...
        \end{enumerate}
    Com isso, vem $f \circ f^{-1} = \id_{\Im{(f)}}$, como queríamos. \blackproof
...
\begin{defi}
    Sejam $(X, \leq)$ um conjunto parcialmente ordenado e $S$ um subconjunto não vazio de $X$.
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Dizemos que $x \in X$ é uma \textit{cota superior} de $S$ se $y \leq x$ para todo $y \in S$. Nesse caso, dizemos que $S$ é \textit{limitado superiormente}.
            \item Dizemos que $x \in X$ é uma \textit{cota inferior} de $S$ se $x \leq y$ para todo $y \in S$. Nesse caso, dizemos que $S$ é \textit{limitado inferiormente}.
            \item Dizemos que $M \in S$ é o \textit{elemento máximo} de $S$ se $y \leq M$ para todo $y \in S$.
            \item Dizemos que $m \in S$ é o \textit{elemento mínimo} de $S$ se $m \leq y$ para todo $y \in S$.
            \item Dizemos que $M \in X$ é o \textit{supremo} de $S$ se $S$ é limitado superiormente e $M \leq y$ para toda cota superior $y \in X$ de $S$.
            \item Dizemos que $m \in X$ é o \textit{ínfimo} de $S$ se $S$ é limitado inferiormente e $y \leq m$ para toda cota inferior $y \in X$ de $S$.
        \end{enumerate}
\end{defi}

\begin{claim}
            $\Dom{(\psi(n))} = n$ para todo $n \in \omega$.
        \end{claim}
        \begin{midproof}
            A prova se dará por indução em $n$.
                Para $n=0$, como $\psi(0) = \emptyset$, temos $\Dom{(\psi(0))} = \Dom{(\emptyset)} = \emptyset = 0$. Suponha, por hipótese de indução, que $\Dom{(\psi(n))} = n$. Observando que
                    \[
                ...
                obtemos
                    ...
                    \]
                e como $\Dom{(\psi(n))} = n$, vem $\Dom{(\psi(n^+))} = n \cup \{ n \} = n^+$. \whiteproof
        \end{midproof}

        Como $\Dom{(\psi(n^+))} = n^+$, temos $n \in \Dom{(\psi(n^+))}$, e se $m \supseteq n^+$, então $\psi(m) \restriction_{n^+} = \psi(n^+)$, de modo que $\varphi(n) = \psi(n^+)(n)$. Como
                \[
                    \psi(n^+) = \psi(n) \cup \{ (n, f(\psi(n)))\}
                \]
            e $n \notin \Dom{(\psi(n))} = n$, só pode ser $\psi(n^+)(n) = f(\psi(n))$. Para finalizarmos, vejamos que $\psi(n) = \varphi \restriction_n$. Por um lado ($\subseteq$), se $(k,x) \in \psi(n)$, então existe $\psi(n) \in \Im{(\psi)}$, com $n \in \omega$, tal que $(k,x) \in \psi(n)$, de modo que $(k,x) \in \bigcup \Im{(\psi)} = \varphi$. Com isso, como $k \in n$, vem $(k,x) \in \varphi \restriction_n$. Por outro lado ($\supseteq$), se $(k,x) \in \varphi \restriction_n$, então existe $m \in \omega$ tal que $(k,x) \in \psi(m)$, e como $k \in n$

\begin{midproof}
            Seja $\Phi : \omega \to X$ tal que $\Phi(n) = f(\Phi \restriction_n)$ para todo $n \in \omega$. Provemos que $\Phi \restriction_n = \psi(n)$ para todo $n \in \omega$ por indução em $n$. 
            Para $n=0$, trivialmente $\Phi \restriction_{0} = \emptyset = \psi(0)$. Suponha, por hipótese de indução, que $\Phi \restriction_n = \psi(n)$. Com isso,
                \begin{align*}
                    \Phi|_{n^+} &= \Phi|_n \cup \{ (n, \Phi(n)) \} \\
                    &= \psi(n) \cup \{ (n, f(\Phi|_n)) \} \\
                    &= \psi(n) \cup \{ (n, f(\psi(n))) \} \\
                    &= h(\psi(n)) = \psi(n^+),
                \end{align*}
            o que completa o passo indutivo. Agora, como $\psi(n) = \varphi \restriction_n$ para todo $n \in \omega$, temos que $\Phi \restriction_n = \varphi \restriction_n$ para todo $n \in \omega$, de modo que
                \[
        ...
        Provemos que $\varphi$ satisfaz a condição de recursão.
    Dado $n \in \omega$, como $\Dom{(\psi(n))} = n$ e $\psi(n) \subseteq \varphi$, temos que $\varphi|_n = \psi(n)$.
    O valor $\varphi(n)$ é determinado por $\psi(n^+)$, visto que $n \in \Dom{(\psi(n^+))}$.
    Assim:
        \[
            \varphi(n) = \psi(n^+)(n) = f(\psi(n)) = f(\varphi|_n).
        \]
\[
                    \varphi = \bigcup \Im{(\psi)} = \bigcup_{n \in \omega} \psi(n) = \bigcup_{n \in \omega} \varphi \restriction_n = \bigcup
                \]
    Para a unicidade, suponha que exista outra função 
    \begin{align*}
                        \Phi|_{n^+} &= \Phi|_n \cup \{ (n, \Phi(n)) \} \\
                        &= \varphi \restriction_n \cup \{ (n, f(\Phi \restriction_n)) \} \\
                        &= \varphi \restriction_n \cup \{ (n, f(\varphi \restriction_n)) \} \\
                        &= h(\varphi \restriction_n) = \psi(n^+).
                \end{align*}
    
    
    tal que $\Phi(n) = f(\Phi|_n)$ para todo $n \in \omega$.
    Mostremos por indução que $\Phi|_n = \psi(n)$.
    Para $n=0$, $\Phi|_0 = \emptyset = \psi(0)$.
    Supondo que $\Phi|_n = \psi(n)$, temos
        \begin{align*}
            \Phi|_{n^+} &= \Phi|_n \cup \{ (n, \Phi(n)) \} \\
                        &= \psi(n) \cup \{ (n, f(\Phi|_n)) \} \\
                        &= \psi(n) \cup \{ (n, f(\psi(n))) \} \\
                        &= h(\psi(n)) = \psi(n^+).
        \end{align*}
    Logo, $\Phi|_n = \psi(n)$ para todo $n \in \omega$.
    Consequentemente, $\Phi = \bigcup_{n \in \omega} \Phi|_n = \bigcup_{n \in \omega} \psi(n) = \varphi$.
    \blackproof
%\end{proof}

\begin{proof} \label{teo.fund:uomega}
    Provemos primeiramente que $\omega(I)$ está bem definido. Pelo axioma das partes, existe o conjunto $\mathcal{P}{(I)}$. Pelo axioma da separação com universo $\mathcal{P}{(I)}$ e a fórmula $ (\emptyset \in x) \land \forall y ((y \in x) \rightarrow (y^+ \in x)  )$ (que se abrevia por $\ind{(x)}$), existe $z(I) :=  \{ x \in \mathcal{P}{(I)} : \ind{(x)} \}$. Pelo \Cref{teo.fund:intfamilia}, como $z(I) \neq \emptyset$ já que $I \in z(I)$, existe $\omega(I) := \bigcap z(I)$.
        \begin{enumerate}[leftmargin=*, align=left, label=\textbf{(\alph*)}]
            \item Provemos primeiramente que $\emptyset \in \omega(I)$. Pelo \Cref{teo.fund:intfamilia}, $\omega(I) := \bigcap z(I)$ é o conjunto que satisfaz
                \[
            ...
            o que equivale a
                ...
                \]
            o que sabemos ser verdade: se $y \subseteq I$ é indutivo, então $\forall x (x \in y \rightarrow x^+ \in y)$. Logo, $\omega{(I)}$ é indutivo. \blackproof
            \item Provemos inicialmente que, para quaisquer $I$ e $J$ indutivos, tem-se $\omega (I) \subseteq J$. O mesmo argumento do item anterior mostra que $I \cap J$ são indutivos, e como $I \cap J \subseteq I$, vem $I \cap J \in z(I)$. Com isso, como
                ...
                \]
            particularmente para $w = I \cap J$, se $x \in \bigcap z(I)$, então $x \in I \cap J$, de modo que $\bigcap{z(I)} \subseteq I \cap J \subseteq J$, isto é, $\omega{(I)} \subseteq J$. Como isso, como $\omega{(I)}$ e $\omega{(J)}$ são indutivos, temos $\omega{(I)} \subseteq \omega{(J)}$ e $\omega{(J)} \subseteq \omega{(I)}$, isto é, $\omega{(I)} = \omega{(J)}$. \blackproof
        \end{enumerate}
\end{proof}